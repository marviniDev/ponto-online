...
const Calendar: React.FC = () => {
    ...
    async function loadData() {
      try {
        const response = await Api.get(
          `/scoreMonth/${user?.id}/${monthCurrent + 1}/${yearCurrent}`
        );
      ...
      document
        .getElementsByClassName("day-marck active")[0]
  ...
  const saveRequest = async () => {
    if (request?.description?.trim()) {
      ...
      let date = _request?.date;
      let input = _request?.input || "00:00";
      let out_lunch = _request?.out_lunch || "00:00";
      let lunch_entree = _request?.lunch_entree || "00:00";
      let exit = _request?.exit || "00:00";
      let description = _request?.description;
      try {
        let response = await Api.post(`/request/${request?.id}`, {
          input: `${date} ${input}:00`,
          out_lunch: `${date} ${out_lunch}:00`,
          lunch_entree: `${date} ${lunch_entree}:00`,
          exit: `${date} ${exit}:00`,
          description: description,
        });

        response.data && response.data?.Request === "true"
          ? toast.current.show({
              severity: "success",
              summary: "Tudo certo!",
    ...
    <Main>
      ...
      >
        ...
        <div className="p-field">
          ...
          />
          {submitted && <small className="p-error">Nome é obrigatorio.</small>}
        </div>

        {
            request && request.date?.slice(0,1) && dayOfWeek(yearCurrent, monthCurrent, parseInt(request.date?.slice(0,1))) !== "Sab" ? (
                <>
                                            <div className="p-field">
